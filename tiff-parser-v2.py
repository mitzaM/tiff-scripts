...
def parse(filename):
    ...
    fields_not_found = [True] * len(CSV_FIELDS)

    for e in tree.getroot().iter():
        tag_name = e.tag[e.tag.rindex("}")+1:]
        if fields_not_found[CSV_FIELDS.index("Cinema")]:
            code = get_code(e.text, tag_name)
            if code:
                result["Cinema"] = code
                fields_not_found[CSV_FIELDS.index("Cinema")] = False
        try:
            idx = XML_TAGS.index(tag_name)
            fields_not_found[idx] = False
            result[CSV_FIELDS[idx]] = func[idx](e.text, XML_TAGS[idx])
        except ValueError:
            pass

        if not any(fields_not_found):
            break

    for i, e in enumerate(fields_not_found):
        e and logging.info("{} not found.".format(CSV_FIELDS[i]))
